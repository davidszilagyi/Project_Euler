...
namespace Project_Euler.Problem_Solver
{
    class Problem_8
    {
        ...
        public long Largest(int count)
        {
            List<string> nums = Split(count);
            long result;
            long final = 0;
            foreach (string num in nums)
            {
                ...
                {
                    result *= (int)Char.GetNumericValue(number);
                }
                if (final < result)
                {
                    final = result;
        ...
        private List<string> Split(int count)
        {
            List<string> numbers = new List<string>();
            for (int i = 0; i < number.Length; i++)
            {
                string result = "";
                if (count + i <= number.Length - 1)
                {
